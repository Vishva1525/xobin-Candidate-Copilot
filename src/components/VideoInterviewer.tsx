...
export function VideoInterviewer({ questionText, role, company, onSpeechEnd, isActive }: VideoInterviewerProps) {
  ...
  const speak = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      // Simulate mouth movement with variation
  ...
  }, [questionText, onSpeechEnd, stopSpeech]);

  // Speak when question changes and is active
  useEffect(() => {
        ...
        <AnimatePresence>
          {isSpeaking && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 8 }}
              className="absolute top-4 right-4 flex items-center gap-2 rounded-full bg-primary/15 backdrop-blur-sm px-3 py-1.5"
            >
...
function AvatarFigure({ mouthOpen, isSpeaking }: { mouthOpen: number; isSpeaking: boolean }) {
  // Subtle idle animation for eyes
